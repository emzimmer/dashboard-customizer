...
*/
		...
		// Typography section
		$title_typography = $title->typographySection( 'Typography', '.eedc-fp-title', $this );
		...
		$li_layout_flex = $li_layout->flex( '.eedc-fp-li', $this );

		// List items links
		$li_link = $list->typographySection( 'Link Typography', '.eedc-fp-link', $this );

		// List items date/time
		$li_datetime = $list->typographySection( 'DateTime Typography', '.eedc-fp-datetime', $this );
			...
			while ( $posts->have_posts() ) :
				...
				echo '<li class="eedc-fp-li">';
					...
					if ( $options['display_date_time'] == 'true' ) :
						...
						echo '<span class="eedc-fp-datetime">' . sprintf( _x( '%1$s, %2$s', 'dashboard' ), $relative, get_the_time() ) . '</span>';
	...
	function defaultCSS() {
		$css = '.oxy-dashboard-customizer-future-posts ul {
					list-style:none;
					padding:0;
					margin:0;
				}
